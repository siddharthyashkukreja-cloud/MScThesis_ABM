"""
Client-clearing contagion experiment on the real COVID crash window.
...
standard methodology (procyclical VaR IM, cash/IM capital ratio, cover-2 SLOIM, deficit
...
# Anchored to real session opens so the window begins on a session boundary and
# spans whole sessions — real sessions are ~405 bars and vary, not a fixed 390.
...
def run(c: float, seed: int = 42, direct: bool = False):
    """Run the COVID window with the observed path amplified by multiplier c (c=1 is
    actual COVID). `direct=True` runs the H1 counterfactual: clients clear directly
    at the CCP, no client tier. Returns the cascade summary."""
    p = ModelParams(
    ...
    ccp = build_clearing_tier(traders, p, seed=seed, direct=direct)
    ...
    return dict(
        ...
        cm_defaults=int(ch[ch["has_defaulted"] & ch["kind"].isin(["BCM", "NBCM"])]
                        ["agent_id"].nunique()) if len(ch) else 0,
...
def _gapped_v(c: float) -> np.ndarray:
    ...
    gaps retained). Same construction as run()."""
    w = _V[WINDOW_START:WINDOW_START + WINDOW_STEPS]
...
def _shock_v(c: float) -> np.ndarray:
    ...
    out[len(g) // 2:] = g.min()             # single jump to the trough at the window midpoint
    return out
...
def scenario_run(kind: str, c: float, seed: int = 42) -> dict:
    ...
    return dict(
        ...
        mutualised_l3=bool(deepest >= 3),   # pooled-DF draw (prefunded survivor cash)
        mutualised_l4=bool(deepest >= 4),   # L4 = surviving-member cash pro-rata
...
def trace(seed: int = 42, direct: bool = False):
    r = run(1.0, seed, direct=direct)
    print(f"COVID window cascade trace (seed {seed}"
          f"{', DIRECT clearing' if direct else ''}): drawdown {r['drawdown']:+.1f}%")
...
if __name__ == "__main__":
    ...
    direct = "direct" in sys.argv[2:]      # e.g. `python3 covid_contagion.py reverse direct`
    if mode == "scenario":
        scenario_sweep()
    elif mode == "reverse":
        reverse(direct=direct)
    else:
        trace(direct=direct)
