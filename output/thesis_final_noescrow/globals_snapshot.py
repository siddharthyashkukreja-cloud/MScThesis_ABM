...
# Default close-out of a defaulted book (CLOSEOUT_MODE):
#   "transfer" (DEFAULT) — the ODD-style resolution: a defaulted client's position is
#       assigned to its CM's largest-offsetting client, and a defaulted CM's book is
#       transferred to the surviving member with the largest opposing position, at
#       CLOSEOUT_RECOVERY (no open-market liquidation, no endogenous price impact). The
#       realised loss is the (1 - recovery) close-out haircut on the transferred position,
#       borne by the CM (client default) or the waterfall (CM default).
#   "firesale" — the open-market Almgren-Chriss liquidation (the CCP/CM sells the assumed
#       book into the LOB; the book-walk price impact is the contagion channel). Kept as a
#       deferred ADDITIONAL HYPOTHESIS (the price-impact channel) together with the
#       reverse-stress amplifier; off by default.
...
# conservative than the Lehman outcome, less than the ODD's credit-style 0.60. Sensitivity-
# test it. (Note: a solvent BCM's capital-ratio DELEVERAGE still trims its own book via AC;
# only the DEFAULT close-out is governed by this toggle.)
...
CCP_CASH = 7_500_000_000            # CCP TOTAL prefunded resources (the L5 backstop /
                                    # default-fund-cap scale, ~LCH SwapClear ~£6B; real
                                    # CME/LCH prefunded DF ~$9-10B). NOTE this is NOT the
                                    # CCP skin-in-the-game: SITG is the ex_df_ratio (~3%)
                                    # slice of the fund (~$100-250M at real CCPs), set in
                                    # CCP_CALIBRATION below.
