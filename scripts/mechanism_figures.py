...
and writes four figures to output/results/ (also copies to FIG_DEST if set):
    mech_position.png        fewer-but-fatter-tailed defaulted positions (the freeze-trap)
    mech_waterfall.png       where the loss goes: layer absorption + mutualisation frequency
    mech_defaults_by_type.png who bears the defaults (FT / MT / ZI)
    mech_freezes.png         freeze channel: CM-contagion vs own-distress
...
for lay, nm, c in zip(layers, lnames, lcols):
    ...
    bottom += vals
ax[0].set_xticks(range(4)); ax[0].set_xticklabels([LABEL[a] for a in ARMS])
ax[0].set_ylabel("loss absorbed per seed (\\$B)")
ax[0].set_title("Deficit by waterfall layer"); ax[0].legend(fontsize=8, loc="upper left")
ax[0].text(0.5, 0.97, "L4–L5 implemented but never engaged", transform=ax[0].transAxes,
           ha="center", va="top", fontsize=8.5, color="#555",
           bbox=dict(boxstyle="round", fc="white", ec=GREY, alpha=0.9)); _sep(ax[0])
...
ax[1].set_ylabel("events reaching the pooled fund / seed")
ax[1].set_title("Mutualisation frequency ($L \\geq 3$)"); _sep(ax[1])
fig.suptitle("Higher margin pushes more (and deeper) defaults into the mutualised waterfall",
             fontsize=12.5, fontweight="bold", y=1.02)
_save(fig, "mech_waterfall.png")
...
for x, c in enumerate(cont):
    ax.text(x, c - 4, f"{c:.0f}%", ha="center", va="top", color="white", fontsize=9, fontweight="bold")
ax.set_xticks(range(4)); ax.set_xticklabels([LABEL[a] for a in ARMS])
